import React, { useState, useEffect } from 'react';
import { connect } from 'react-redux';
import { Link, RouteComponentProps } from 'react-router-dom';
import { Button, Col, Row, Table } from 'reactstrap';
import { Translate, ICrudGetAllAction, TextFormat } from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

import { IRootState } from 'app/shared/reducers';
import { getEntities } from './state.reducer';
import { IState } from 'app/shared/model/state.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IStateProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const State = (props: IStateProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { stateList, match, loading } = props;
  return (
    <div>
      <h2 id="state-heading">
        <Translate contentKey="gadiApp.state.home.title">States</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="gadiApp.state.home.createLabel">Create new State</Translate>
        </Link>
      </h2>
      <div className="table-responsive">
        {stateList && stateList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="global.field.id">ID</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.name">Name</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.identifier">Identifier</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.orderValue">Order Value</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.defaultValue">Default Value</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.active">Active</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.createdBy">Created By</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.createdDate">Created Date</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.updatedBy">Updated By</Translate>
                </th>
                <th>
                  <Translate contentKey="gadiApp.state.updatedDate">Updated Date</Translate>
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {stateList.map((state, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${state.id}`} color="link" size="sm">
                      {state.id}
                    </Button>
                  </td>
                  <td>{state.name}</td>
                  <td>{state.identifier}</td>
                  <td>{state.orderValue}</td>
                  <td>{state.defaultValue ? 'true' : 'false'}</td>
                  <td>{state.active ? 'true' : 'false'}</td>
                  <td>{state.createdBy}</td>
                  <td>{state.createdDate ? <TextFormat type="date" value={state.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{state.updatedBy}</td>
                  <td>{state.updatedDate ? <TextFormat type="date" value={state.updatedDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${state.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${state.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${state.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.delete">Delete</Translate>
                        </span>
                      </Button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </Table>
        ) : (
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gadiApp.state.home.notFound">No States found</Translate>
            </div>
          )
        )}
      </div>
    </div>
  );
};

const mapStateToProps = ({ state }: IRootState) => ({
  stateList: state.entities,
  loading: state.loading,
});

const mapDispatchToProps = {
  getEntities,
};

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = typeof mapDispatchToProps;

export default connect(mapStateToProps, mapDispatchToProps)(State);
